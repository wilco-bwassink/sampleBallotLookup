<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			electionData = normalizeElectionData(data as ElectionPayload);
			elections = Object.values(electionData)
	...
	function getElectionLabel(key: string): string {
		...
		if (!rec) return key;
		if (isES) {
			return rec.ElectionNameSpanish || rec.ElectionName || key;
		}

		return rec.ElectionName || key;
	}
